...
def test_create_output_dir():
    assert regridder.create_output_dir('/tmp/out', domain_type=regridder.GLOBAL) == '/tmp/out/1_deg'
    assert regridder.create_output_dir('/tmp/out', domain_type=regridder.REGIONAL) == '/tmp/out/0.5_deg'


def test_get_grid_definition_file():
    assert 'grid_files/ll1deg_grid.nc' in regridder.get_grid_definition_file(
        CMIP5_NC, domain_type=regridder.GLOBAL)
    assert 'grid_files/ll0.5deg_AFR-44i.nc' in regridder.get_grid_definition_file(
        CORDEX_NC, domain_type=regridder.REGIONAL)
...
def test_validate_regridded_file_coredex():
    regridder.validate_regridded_file(CORDEX_NC, regridder.REGIONAL)


@pytest.mark.skip(reason='not working')
def test_validate_regridded_file_cmip5():
    regridder.validate_regridded_file(CMIP5_NC, regridder.GLOBAL)


def test_regrid_cordex():
    assert regridder.regrid(CORDEX_NC, regridder.REGIONAL, archive_base=ARCHIVE_BASE) == \
        'OUT/0.5_deg/tasmin_AFR-44i_ECMWF-ERAINT_evaluation_r1i1p1_MOHC-HadRM3P_v1_mon_199001-199012.nc'


@pytest.mark.skip(reason='not working')
def test_regrid_cmip5():
    assert regridder.regrid(CMIP5_NC, regridder.GLOBAL, archive_base=ARCHIVE_BASE) == ''
